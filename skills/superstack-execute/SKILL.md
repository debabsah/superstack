...
A cold fresh-context subagent (general-purpose — it must run checks, so the read-only reviewer agent cannot audit) re-runs the milestone's checks, reads the done-whens cold, inspects the artifact against them. **Auditor-red beats builder-green.** Closure needs a PASSED audit or an owner waiver — owner-authored, quoting the owner, naming the skipped done-when. "Closed" means audited-or-waived; otherwise BUILT-UNAUDITED, **and the frontier never passes two unaudited milestones.** Audit verdicts append to the plan; failures mint gotchas.
...
**Then run superstack-ship's calibration-record steps against the campaign** — its steps 1–3, in its order. `ship` retires task files and knows nothing of plans, so closure here is the only path that reaches the end without them (D-10).
