...
1. **Append** every `Verified:` line from the final report to `.superstack/claims-log` (`date · claim · command`) — this is what `superstack-debug` falsifies against when a vouched-for behavior later breaks.
