...
check "outward tallies count t3 bounces too" "outward: 2 bounce" "$out"
check "gate run denominators reported"      "runs counted: claims 214 look 214 outward 3" "$out"
...
# a growing run counter IS a hook trace: it must silence the dark warning and
# surface the denominator even before any row has landed in a log
printf 'claims 12\n' > "$shl/gate-runs.claims"
out="$( cd "$hl" && bash "$doctor" 2>&1 )"
if printf '%s' "$out" | grep -q 'no gate, outward, or load line'; then fail=$((fail+1)); echo "FAIL: workspace with a run counter still flagged hooks-dark"
else pass=$((pass+1)); echo "PASS: a run counter counts as a hook trace"; fi
check "denominator shown without any gate-log"       "runs counted: claims 12" "$out"
